...
export default function FriendsLoginPage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const response = await fetch(mode === "create" ? "/api/auth/register" : "/api/auth/login", {
        method: "POST",
        headers: { "Content-Type": "application/json", "x-pickem-group": "friends" },
        body: JSON.stringify({ username, password, group: "friends" })
      });
  ...
  return <main className="app-shell login-screen login-friends">
    <section className="login-card">
      <div className="login-app-name">Friends Pick&apos;em</div>
