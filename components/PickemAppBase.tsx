...
function matchupTextVariants(game: Game, options: { spreadTeam?: string; spread?: number | null; marketText?: string; suffix?: string; intermediateSuffix?: string; compactSuffix?: string } = {}) {
  ...
  const marketText = options.marketText;
  const format = (away: string, home: string, suffix: string) => {
    const teamMarket = marketText ?? spreadText(spread);
    const awayMarket = spreadTeam === game.away_team ? ` ${teamMarket}` : "";
    const homeMarket = spreadTeam === game.home_team ? ` ${teamMarket}` : "";
...
export default function PickemApp({ appSlug = "shaw-family" }: { appSlug?: AppSlug }) {
  ...
  useEffect(() => {
    const image = headerLogoRef.current;
    if (image?.complete && image.naturalWidth > 0) setHeaderLogoReady(true);
  }, [appSlug, loading]);
  ...
  }, [tab, picksView, cardView, standingsView, betView, sideBetLedgerScope, sideBetLedgerGameFilter, data?.week, data?.sideBets.length, sideBetLedger.length]);
    ...
    <main className="container">
      ...
      {tab === "standings" && <section className="panel standings-panel">
        ...
        {standingsView === "bank" && <>
          ...
          <div className="subsection bank-section bank-week-section"><div className="standings-heading-row side-bet-ledger-heading-row"><h2 className="heading-with-badge">Side Bet Ledger <span className={`side-bet-week-total ${mySideBetWeekTotal > 0 ? "money-pos" : mySideBetWeekTotal < 0 ? "money-neg" : "money-neutral"}`.trim()}><NumericText text={money(mySideBetWeekTotal)} /></span><NotificationBadge count={bankNotificationCount} /></h2></div><div className="side-bet-ledger-filter-row"><div className="side-bet-ledger-scope-toggle" role="group" aria-label="Filter side bet ledger"><button type="button" className={sideBetLedgerScope === "all" ? "active" : ""} aria-pressed={sideBetLedgerScope === "all"} onClick={() => setSideBetLedgerScope("all")}>All Bets</button><button type="button" className={sideBetLedgerScope === "mine" ? "active" : ""} aria-pressed={sideBetLedgerScope === "mine"} onClick={() => setSideBetLedgerScope("mine")}>My Bets</button></div><div className="side-bet-game-filter-toggle" role="group" aria-label="Filter side bets by game status"><button type="button" className={sideBetLedgerGameFilter === "all" ? "active" : ""} aria-pressed={sideBetLedgerGameFilter === "all"} onClick={() => setSideBetLedgerGameFilter("all")}>All</button><button type="button" className={sideBetLedgerGameFilter === "upcoming" ? "active" : ""} aria-pressed={sideBetLedgerGameFilter === "upcoming"} onClick={() => setSideBetLedgerGameFilter("upcoming")}>Upcoming</button><button type="button" className={sideBetLedgerGameFilter === "completed" ? "active" : ""} aria-pressed={sideBetLedgerGameFilter === "completed"} onClick={() => setSideBetLedgerGameFilter("completed")}>Completed</button></div></div><div className="ledger-list">{!previewActive && !sideBetLedgerReady && <p className="muted">Loading side bet ledger…</p>}{(previewActive || sideBetLedgerReady) && viewedSideBetLedger.length === 0 && <p className="muted ledger-empty-state">No side bets in the ledger yet.</p>}{(previewActive || sideBetLedgerReady) && viewedSideBetLedger.map((bet) => <SideBetLedgerRow key={bet.id} bet={bet} currentUser={currentUser} />)}</div></div>
        </>}
      </section>}
...
function sideBetGameCompleted(bet: SideBet) {
  const game = bet.game;
  return bet.status === "settled" || Boolean(game?.live_completed) ||
    Boolean(game && game.final_home_score != null && game.final_away_score != null);
}

function sideBetMatchesGameFilter(bet: SideBet, filter: SideBetGameFilter) {
  if (filter === "all") return true;
  const completed = sideBetGameCompleted(bet);
  return filter === "completed" ? completed : !completed;
}

function sideBetKickoffTime(bet: SideBet) {
  const kickoff = bet.game?.commence_time ? new Date(bet.game.commence_time).getTime() : Number.POSITIVE_INFINITY;
  return Number.isFinite(kickoff) ? kickoff : Number.POSITIVE_INFINITY;
}

function sideBetCompletionTime(bet: SideBet) {
  const updated = new Date(bet.updated_at || bet.accepted_at || bet.created_at).getTime();
...
function SideBetLedgerRow({ bet, currentUser }: { bet: SideBet; currentUser: Profile }) {
  ...
  return <div className={`ledger-row side-bet-ledger-row ${bet.status === "accepted" ? "accepted" : ""}`}>
    ...
    <div className="side-bet-ledger-copy"><strong className="side-bet-ledger-title"><ResponsiveText full={matchup.full} intermediate={matchup.intermediate} compact={matchup.compact} className="side-bet-ledger-matchup" /></strong><p><span>{bettors}</span>{status ? <> · <span className={statusTone}>{status}</span></> : null}</p></div>
    {amountDisplay.settled || amountDisplay.evenPayout
