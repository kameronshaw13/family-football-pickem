...
function TeamLogo({ src, name, size = 40 }: { src?: string | null; name: string; size?: number }) {
  return src ? <Image unoptimized src={src} alt="" width={size} height={size} /> : <span className="game-tracker-logo-fallback">{name.slice(0, 2).toUpperCase()}</span>;
}
...
function TeamScore({ side, payload, game }: { side: Side; payload: TrackerPayload | null; game: Game }) {
  ...
  return <div className="game-tracker-score-team">
    <TeamLogo src={team?.logo || fallbackLogo} name={team?.name || fallbackName} size={44} />
    <div><strong>{team?.name || fallbackName}</strong><small>{side === "away" ? "AWAY" : "HOME"}</small></div>
...
function LiveField({ payload }: { payload: TrackerPayload }) {
  ...
  return <div className="game-tracker-live">
    ...
    <div className="game-tracker-field-footer">
      ...
      <strong>{possession ? `${possession.abbreviation || possession.name} driving` : statusText(payload, {} as Game)}</strong>
...
function Scoring({ payload }: { payload: TrackerPayload }) {
  ...
  return <div className="game-tracker-scoring-list">{payload.scoringPlays.map(play => {
    const side = play.teamSide;
    const team = side ? payload.teams[side] : null;
    return <div className="game-tracker-score-play" key={play.id}>
      <span className="game-tracker-score-logo">{team && <TeamLogo src={team.logo} name={team.name} size={30} />}</span>
      <div><small>{[play.period, play.clock, play.type].filter(Boolean).join(" · ")}</small><strong>{play.text}</strong></div>
...
function DriveSummary({ drive, payload }: { drive: TrackerDrive; payload: TrackerPayload }) {
  ...
  return <summary>
    ...
    <div><strong>{drive.current ? "Current Drive" : drive.result || "Drive"}</strong><small>{detail || drive.description || "Drive summary"}</small></div>
...
function Plays({ payload }: { payload: TrackerPayload }) {
  ...
  return <div className="game-tracker-drives">{payload.drives.map((drive, index) =>
    <details className="game-tracker-drive" key={drive.id} open={drive.current || index === 0}>
      ...
      <div className="game-tracker-drive-plays">
        {drive.plays.length ? drive.plays.map(play => <div className="game-tracker-drive-play" key={play.id}>
          <span>{[play.period, play.clock].filter(Boolean).join(" · ")}</span>
          <div><strong>{play.text}</strong>{play.situation && <small>{play.situation}</small>}</div>
...
function TeamStats({ payload }: { payload: TrackerPayload }) {
  return <section className="game-tracker-team-stats">
    <div className="game-tracker-stat-table-head"><span>{payload.teams.away.abbreviation || "Away"}</span><strong>TEAM STATS</strong><span>{payload.teams.home.abbreviation || "Home"}</span></div>
...
function PlayerTable({ row, teamName }: { row: TrackerPayload["playerStats"][number]; teamName: string }) {
  const visibleLabels = row.labels.slice(0, 6);
  return <div className="game-tracker-player-table-wrap">
    <div className="game-tracker-player-table-team">{teamName}</div>
    <table className="game-tracker-player-table">
      <thead><tr><th>Player</th>{visibleLabels.map(label => <th key={label}>{label}</th>)}</tr></thead>
      <tbody>{row.athletes.map(athlete => <tr key={athlete.name}><td>{athlete.name}</td>{visibleLabels.map((_, index) => <td key={index}>{athlete.values[index] ?? "—"}</td>)}</tr>)}</tbody>
    </table>
  </div>;
}
  ...
  return <div className="game-tracker-box">
    ...
    {categories.map(category => {
      ...
      return <section className="game-tracker-box-section" key={category}>
        <h3>{category}</h3>
        {away && <PlayerTable row={away} teamName={payload.teams.away.abbreviation || payload.teams.away.name} />}
        {home && <PlayerTable row={home} teamName={payload.teams.home.abbreviation || payload.teams.home.name} />}
      </section>;
    })}
  </div>;
}
  ...
  return createPortal(<div className="game-tracker-backdrop" onClick={event => { if (event.target === event.currentTarget) onClose(); }}>
    <section ref={sheet} className="game-tracker-sheet" role="dialog" aria-modal="true" aria-label={`${awayName} at ${homeName} GameTracker`}>
      ...
      <div className="game-tracker-scoreboard">
        <div className="game-tracker-status-line"><span className={payload?.status.completed ? "final" : "live"}>{payload?.status.completed ? "FINAL" : "LIVE"}</span><strong>{statusText(payload, game)}</strong></div>
