...
function spreadText(value?: number) {
  ...
  if (!Number.isFinite(n) || n === 0) return "Pick'em";
...
function responseTarget(bet: Bet) {
  if (bet.accepted_by) return bet.targets?.find((target) => target.recipient_id === bet.accepted_by) || null;
...
function readSeen(userId: string) {
  try {
    return new Set((JSON.parse(window.localStorage.getItem(seenKey(userId)) || "[]") as string[]).filter(Boolean));
  } catch {
...
function addSeen(userId: string, ids: string[]) {
  ...
  try {
    window.localStorage.setItem(seenKey(userId), JSON.stringify(Array.from(seen)));
  } catch {
    // Local storage is only a convenience for one-look accepted-card behavior.
  }
}

function cardNames(bet: Bet) {
  return {
...
function teamDisplayName(bet: Bet, team?: string) {
  const names = cardNames(bet);
  if (team === bet.game?.away_team) return names.away;
  if (team === bet.game?.home_team) return names.home;
...
function setCardLogo(card: HTMLElement, bet: Bet, team?: string) {
  ...
  if (host instanceof HTMLImageElement && url) {
    if (host.src !== url) host.src = url;
    host.alt = "";
  }
}

function responsePresentation(bet: Bet, mode: BetMode, userId: string) {
  const offeredLine = `${teamDisplayName(bet, bet.offered_team)} ${spreadText(bet.offered_spread)}`;
  ...
  if (bet.status === "accepted" || target?.response === "accepted") {
    const actor = (bet.accepted_by || target?.recipient_id) === userId
      ? "You"
      : bet.accepted_by_profile?.display_name || target?.recipient?.display_name || recipientNames;
    return `${actor} Accepted ${offeredLine}`;
  }
  if (bet.status === "declined" || target?.response === "declined") {
    ...
    return `${actor} Declined ${offeredLine}`;
  }
  if (bet.status === "cancelled") {
    return `${bet.creator_id === userId ? "You" : creatorName} Cancelled ${offeredLine}`;
  }
  if (bet.status === "expired") return `Offer Expired ${offeredLine}`;
  if (mode === "received") return `${creatorName} Offered ${offeredLine}`;
  return `You Offered ${recipientNames} ${offeredLine}`;
}

function mainPresentation(bet: Bet, userId: string) {
...
function renderCards(payload: AppData, mode: BetMode, seen: Set<string>) {
  ...
  const list = cards[0]?.closest<HTMLElement>(".side-bet-list") || null;
  ...
  cards.forEach((card, index) => {
    ...
    const hidden = bet.status === "accepted" && seen.has(bet.id);
    card.hidden = hidden;
    if (!hidden && bet.status === "accepted") shown.push(bet.id);

    const isCreator = bet.creator_id === userId;
    const perspectiveTeam = isCreator ? bet.creator_team : bet.offered_team;
    setCardLogo(card, bet, perspectiveTeam);

    const main = card.querySelector<HTMLElement>(".side-bet-offer-copy > strong");
    if (main) main.dataset.sideBetRuntimeText = mainPresentation(bet, userId);
    const response = card.querySelector<HTMLElement>(".side-bet-offer-copy p");
    if (response) {
      const kickoff = kickoffText(bet.game?.commence_time);
      response.dataset.sideBetRuntimeText = [responsePresentation(bet, mode, userId), kickoff].filter(Boolean).join(" · ");
    }
  });
  return shown;
}
...
function addReviewLogos(payload: AppData, reviewBetId: string | null) {
  ...
  [bet.offered_team, bet.creator_team].forEach((team, index) => {
...
function bettorForTeam(bet: Bet, team?: string) {
  ...
  if (team === bet.offered_team) {
    return {
      id: bet.accepted_by || responseTarget(bet)?.recipient_id || "",
      name: bet.accepted_by_profile?.display_name || responseTarget(bet)?.recipient?.display_name || "Opponent"
    };
  }
  return null;
}
...
function ledgerPresentation(bet: Bet, userId: string) {
  const game = bet.game;
  const away = game?.away_team || bet.offered_team || "Away";
  const home = game?.home_team || bet.creator_team || "Home";
  const awayName = fallbackDisplayName(away, game?.league);
  const homeName = fallbackDisplayName(home, game?.league);
  const involvesUser = bet.creator_id === userId || bet.accepted_by === userId;
  const userTeam = bet.creator_id === userId ? bet.creator_team : bet.accepted_by === userId ? bet.offered_team : undefined;
  const favoriteTeam = Number(bet.creator_spread) < 0
    ? bet.creator_team
    : Number(bet.offered_spread) < 0
      ? bet.offered_team
      : bet.creator_team;
  const displayTeam = involvesUser ? userTeam : favoriteTeam;
  const displaySpread = displayTeam === bet.creator_team ? bet.creator_spread : bet.offered_spread;
  const title = displayTeam === away
    ? `${awayName} ${spreadText(displaySpread)} at ${homeName}`
    : `${awayName} at ${homeName} ${spreadText(displaySpread)}`;
  const person = (profile: { id?: string; name?: string } | null) => profile?.id === userId ? "You" : profile?.name || "Player";
  return {
    displayTeam,
    title,
    names: `${person(bettorForTeam(bet, away))} vs ${person(bettorForTeam(bet, home))}`
  };
}
...
function renderLedger(payload: AppData, ledgerBets: Bet[]) {
  ...
  const rows = ledgerBets
    .filter((bet) => bet.status === "accepted" || bet.status === "settled")
    .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
  const signature = `${userId}|${rows.map((bet) => `${bet.id}:${bet.status}:${bet.result || ""}:${bet.winner_id || ""}`).join("|")}`;
  if (ledger.dataset.fullSideBetLedgerSignature === signature && ledger.querySelector(".runtime-full-ledger")) return;
  ledger.dataset.fullSideBetLedgerSignature = signature;

  Array.from(ledger.children).forEach((node) => {
    if (node instanceof HTMLElement) node.hidden = true;
  });
  ledger.querySelectorAll(".runtime-full-ledger").forEach((node) => node.remove());

  rows.forEach((bet) => {
    const presentation = ledgerPresentation(bet, userId);
    const row = document.createElement("div");
    row.className = "ledger-row side-bet-ledger-row runtime-full-ledger";
    row.dataset.sideBetId = bet.id;
    row.hidden = false;

    const logo = logoForTeam(bet.game, presentation.displayTeam);
    if (logo) {
      ...
      fallback.textContent = fallbackDisplayName(presentation.displayTeam, bet.game?.league).slice(0, 1);
      row.append(fallback);
    }
    ...
    const winnerName = bet.winner_id === userId
      ? "You"
      : bet.winner_id === bet.creator_id
        ? bet.creator?.display_name || "Player"
        : bet.accepted_by_profile?.display_name || "Opponent";
    const status = bet.status === "accepted"
      ...
      : bet.result === "push"
        ...
        : bet.winner_id
          ? `${winnerName} Won`
          : "Settled";
    meta.textContent = `${presentation.names} · ${status}`;
    copy.append(title, meta);

    const amount = document.createElement("strong");
    const involvesUser = bet.creator_id === userId || bet.accepted_by === userId;
    if (bet.status === "settled" && involvesUser && bet.result !== "push" && bet.winner_id) {
      const won = bet.winner_id === userId;
      amount.className = won ? "money-pos" : "money-neg";
      amount.textContent = `${won ? "+" : "-"}${stakeText(bet.amount)}`;
    } else {
  ...
  if (!rows.length) {
    ...
    empty.className = "muted runtime-full-ledger";
    empty.textContent = "No side bets in the ledger yet.";
    ledger.append(empty);
  }
}
...
export default function SideBetDisplayGuard() {
  useEffect(() => {
    ...
    let acceptedShown: string[] = [];
    let reviewBetId: string | null = null;
    let applying = false;
    let refreshing = false;
    let forceModeRefresh = false;
    ...
    function hideCurrentList() {
      const list = document.querySelector<HTMLElement>(".side-bet-list");
    ...
    function apply() {
      ...
      try {
        ...
        if (previousMode && previousMode !== mode && acceptedShown.length) addSeen(userId, acceptedShown);
        if (forceModeRefresh && mode) hideCurrentList();
        else acceptedShown = mode ? renderCards(payload, mode, readSeen(userId)) : [];
        previousMode = mode;
        addReviewLogos(payload, reviewBetId);
        renderLedger(payload, ledgerBets);
      } finally {
        applying = false;
      }
    }
    ...
    async function refresh(force = false) {
      ...
      refreshing = true;
      try {
        const needsApp = Boolean(document.querySelector(".side-bet-center, .confirmation-sheet")) || !payload;
        const needsLedger = Boolean(document.querySelector(".ledger-list"));
        const [appResponse, ledgerResponse] = await Promise.all([
          needsApp
            ? fetch("/api/app-data", { headers: { Authorization: `Bearer ${token}`, "x-pickem-group": groupSlug() }, cache: "no-store" })
            : Promise.resolve(null),
          needsLedger
            ? fetch("/api/side-bet-ledger", { headers: { Authorization: `Bearer ${token}`, "x-pickem-group": groupSlug() }, cache: "no-store" })
            : Promise.resolve(null)
        ]);
        if (appResponse?.ok) payload = await appResponse.json() as AppData;
        if (ledgerResponse?.ok) {
          const result = await ledgerResponse.json() as { sideBetLedger?: Bet[] };
          ledgerBets = result.sideBetLedger || [];
        }
        forceModeRefresh = false;
        apply();
      } catch {
        if (force) forceModeRefresh = false;
      } finally {
        refreshing = false;
      }
    }
    ...
    const observer = new MutationObserver(() => window.requestAnimationFrame(() => {
      const mode = activeMode();
      if (payload && mode && mode !== previousMode) {
        forceModeRefresh = true;
        hideCurrentList();
        void refresh(true);
        return;
      }
    ...
    return () => {
      if (payload?.currentUser?.id && previousMode && acceptedShown.length) addSeen(payload.currentUser.id, acceptedShown);
      stopped = true;
      observer.disconnect();
